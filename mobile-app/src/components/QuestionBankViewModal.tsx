...
export default function QuestionBankViewModal({
    ...
    // Handle native back button to close this modal
    ...
    const handleImageUpload = useCallback((partName: string, idx: number, file: File) => {
        questionBankApi.uploadImage(bank.id, file)
            .then((res: any) => { updateQuestion(partName, idx, 'imageData', res.data.url); })
            .catch(() => {
    ...
    const handleSave = async () => {
        ...
        setSaveError('');
        try {
            await questionBankApi.updateQuestions(bank.id, editedParts);
            setIsEditing(false);
            setEditedParts({});
            if (onUpdate) onUpdate({ ...bank, questions: { parts: editedParts } });
    ...
    const handleApprove = async () => {
        ...
        setIsApproving(true);
        try {
            await questionBankApi.updateStatus(bank.id, 'APPROVED');
    ...
    const handleReject = async () => {
        ...
        setIsApproving(true);
        try {
            await questionBankApi.updateStatus(bank.id, 'REJECTED');
            setIsApproving(false);
            onClose();
            window.location.reload();
        } catch {
    ...
    return createPortal(
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-2 sm:p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 rounded-xl w-full max-w-full sm:max-w-4xl h-[95vh] sm:h-auto max-h-[95vh] sm:max-h-[90vh] lg:max-h-[85vh] flex flex-col shadow-2xl border border-pink-200 dark:border-pink-700 transform transition-all scale-100 opacity-100 overflow-hidden">

                {/* Fixed Header */}
                <div className="flex-none relative flex flex-col md:flex-row md:justify-between md:items-center p-4 sm:p-6 gap-4 border-b border-pink-200 dark:border-pink-700 bg-white dark:bg-slate-900 rounded-t-xl z-10">
                    <div className="pr-8 md:pr-0 min-w-0 flex-1">
                        <h2 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white break-words min-w-0">{bank.title}</h2>
                        <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 mt-1 break-words min-w-0">{subjectName}</p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full md:w-auto">
                        ...
                        {canEdit && !isEditing && (
                            <button onClick={enterEdit} className="btn btn-secondary px-3 py-2 gap-1.5 text-xs sm:text-sm w-full sm:w-auto justify-center">
                                <Pencil className="w-4 h-4" />Edit
                            </button>
                        )}
                    </div>
                    <button onClick={onClose} className="absolute top-4 right-4 md:static p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200 transition-all flex-shrink-0" aria-label="Close modal">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                {/* Part tabs */}
                {partNames.length > 1 && (
                    <div className="flex-none flex gap-1 px-4 sm:px-6 pt-4 pb-0 bg-white dark:bg-slate-900 border-b border-pink-100 dark:border-pink-900 overflow-x-auto no-scrollbar">
                        {partNames.map(p => (
                            <button
                                key={p}
                ...
                {unitNumbers.length > 0 && (
                    <div className="flex-none flex gap-1 px-4 sm:px-6 pt-3 pb-0 bg-white dark:bg-slate-900 overflow-x-auto no-scrollbar">
                        {unitNumbers.map(u => (
                            <button
                                key={u}
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 bg-slate-50/50 dark:bg-slate-950/30">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                        <span className="text-xl">{isEditing ? '✏️' : activeTab === 'questions' ? '📖' : '📝'}</span>
                        <h3 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white border-b-2 border-pink-200 dark:border-pink-800 pb-1 pr-4">
                ...
                {/* Fixed Footer */}
                <div className="flex-none border-t border-pink-200 dark:border-pink-700 p-4 sm:p-6 bg-white dark:bg-slate-900 rounded-b-xl z-10">
                    {isEditing ? (
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 w-full">
                            {saveError && <p className="text-sm text-rose-600 dark:text-rose-400 text-center sm:text-left">{saveError}</p>}
                            <div className="flex gap-3 justify-end w-full sm:w-auto">
                                <button onClick={cancelEdit} disabled={saving}
                                    className="px-4 py-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-sm font-medium flex-1 sm:flex-none text-center">Cancel</button>
                                <button onClick={handleSave} disabled={saving}
                                    className="btn bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-lg shadow-purple-500/30 hover:scale-105 active:scale-95 transition-all flex-1 sm:flex-none justify-center">
                        ...
                        <div className="flex justify-end w-full">
                            <button onClick={onDownload}
                                className="btn bg-gradient-to-r from-pink-500 via-pink-600 to-purple-600 text-white shadow-lg shadow-pink-500/30 hover:scale-105 active:scale-95 transition-all w-full sm:w-auto justify-center">
        ...
        {showSuccessPopup && createPortal(
            <div className="fixed inset-0 z-[2147483647] flex items-center justify-center p-4 bg-black/60">
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-8 max-w-[95vw] sm:max-w-md w-full text-center border-2 border-emerald-100 dark:border-emerald-900 transform scale-100 transition-all">
...
function ReadonlyQuestionCard({ q, idx, activeTab }: { q: Question; idx: number; activeTab: 'questions' | 'answers' }) {
    return (
        <div className="p-4 sm:p-5 bg-white dark:bg-slate-900 border border-pink-200 dark:border-pink-700/50 rounded-xl hover:shadow-md hover:border-pink-300 dark:hover:border-pink-500 transition-all duration-300">
            ...
            <p className="text-slate-800 dark:text-slate-200 leading-relaxed text-[15px] font-medium pl-0 sm:pl-10 break-words min-w-0 overflow-hidden">
                {q.question}
            </p>

            {q.isMCQ && q.options && (
                <div className="mt-4 pl-0 sm:pl-10 grid grid-cols-1 md:grid-cols-2 gap-3">
                    {Object.entries(q.options).map(([key, value]) => (
                        <div
                            key={key}
                            className={`p-3 rounded-lg text-sm border transition-colors break-words min-w-0 overflow-hidden ${activeTab === 'answers' && q.correctOption === key
            ...
            {activeTab === 'answers' && q.answer && (
                <div className="mt-4 pl-0 sm:pl-10 border-t border-slate-100 dark:border-slate-800 pt-4 break-words min-w-0 overflow-hidden">
