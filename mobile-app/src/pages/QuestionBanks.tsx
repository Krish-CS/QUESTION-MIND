...
export default function QuestionBanks() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<{ open: boolean; bankId: string | null }>({ open: false, bankId: null });
  ...
  const [shareResult, setShareResult] = useState<{ shared_with: string[]; failed: string[]; drive_link?: string | null } | null>(null);
  ...
  useEffect(() => {
    if (!latestBankId) return;
    const timeout = setTimeout(() => setLatestBankId(null), 12000);
    return () => clearTimeout(timeout);
  }, [latestBankId]);

  useEffect(() => {
    if (!patternSaved) return;
    const timeout = setTimeout(() => setPatternSaved(false), 3000);
    return () => clearTimeout(timeout);
  }, [patternSaved]);
  ...
  const loadData = async () => {
    try {
      ...
      const banksData = getData(results[1], [] as QuestionBank[]);

      if (isHOD) {
        const subjectsData = getData(results[2], [] as Subject[]);
        const pendingData = getData(results[3], [] as QuestionBank[]);
        const syllabusMap: Record<string, Syllabus> = {};
        syllabusData.forEach((s: Syllabus) => { syllabusMap[s.subject_id] = s; });
        setSyllabi(syllabusMap);
        setQuestionBanks(banksData);
        setSubjects(subjectsData);
        setPendingApprovals(pendingData.filter((b: QuestionBank) => b.generated_by !== user?.id));
      } else {
        const assignmentsData = getData(results[2], [] as MySubjectAssignment[]);
        const pendingData = getData(results[3], [] as QuestionBank[]);
        const syllabusMap: Record<string, Syllabus> = {};
        syllabusData.forEach((s: Syllabus) => { syllabusMap[s.subject_id] = s; });
        setSyllabi(syllabusMap);
  ...
  const fetchSubjectPattern = async (subjectId: string) => {
    ...
    try {
      ...
      setError('Failed to fetch latest pattern. Using saved configuration if available.');
  ...
  const saveLocalPattern = async () => {
    ...
    try {
      ...
      setError(e?.response?.data?.detail || e?.message || 'Failed to save pattern');
    } finally {
    ...
    setLocalUnitCfg(prev => {
      ...
      if (field === 'mcqCount') {
        ...
        if (Number(value) > total) {
          setBtlWarning(`MCQ count cannot exceed ${total} total questions.`);
          return prev;
        }
      }
  ...
  const handleDownload = async (bank: QuestionBank) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || err.message || 'Failed to download');
    } finally {
  ...
  const handleDelete = async (id: string) => {
    setConfirmDelete({ open: true, bankId: id });
  };

  const confirmDeleteAction = async () => {
    const id = confirmDelete.bankId;
    if (!id) return;
    setConfirmDelete({ open: false, bankId: null });
    try {
  ...
  const getSubjectName = (subjectId: string) =>
    subjectNameById.get(subjectId) || 'Unknown';
  ...
  return (
    <div className="space-y-6">
      {/* BTL overflow warning toast - rendered via portal so it floats above everything */}
      {btlWarning && createPortal(
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[9999] flex items-start gap-3 px-5 py-4 bg-rose-600 text-white rounded-xl shadow-2xl shadow-rose-500/40 max-w-[95vw] sm:max-w-md w-full animate-in fade-in slide-in-from-top-3 duration-300">
          <AlertCircle className="w-5 h-5 flex-shrink-0 mt-0.5" />
          <p className="text-sm font-medium leading-snug flex-1">{btlWarning}</p>
          <button onClick={() => setBtlWarning('')} className="ml-2 p-1.5 rounded-lg bg-rose-700/50 hover:bg-rose-700 text-white/90 hover:text-white transition-colors flex-shrink-0">
            <X className="w-4 h-4" />
          </button>
        </div>,
        document.body
      ...
      {error && (
        <div className="p-4 bg-rose-50 border border-rose-200 rounded-lg text-rose-700 dark:bg-rose-900 dark:border-rose-800 dark:text-rose-200 flex items-start gap-3">
          ...
          <button
            type="button"
            onClick={() => setError('')}
            className="p-1.5 rounded-lg bg-rose-100/50 hover:bg-rose-100 dark:bg-rose-900/50 dark:hover:bg-rose-800 text-rose-600/80 hover:text-rose-700 dark:text-rose-300/80 dark:hover:text-rose-200 transition-colors flex-shrink-0"
                  ...
                  {isDropdownOpen && (
                    <>
                      ...
                      <div
                        id="subject-dropdown"
                        role="listbox"
                        className="absolute z-20 w-full mt-2 bg-white dark:bg-slate-900 border-2 border-pink-100 dark:border-pink-700 rounded-xl shadow-xl max-h-60 overflow-y-auto animate-in fade-in zoom-in-95 duration-200"
                      >
                        {availableSubjects.map(s => (
                          <button
                            ...
                            className={`px-4 py-3 cursor-pointer transition-colors flex items-center justify-between group w-full text-left ${selectedSubjectId === s.id
                ...
                <div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                  ...
                  {hasConfig ? (
                    <div className="bg-white dark:bg-slate-900 rounded-lg border-2 border-pink-200 dark:border-pink-700 overflow-hidden">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between px-4 py-3 bg-pink-50 dark:bg-pink-900/20 border-b border-pink-100 dark:border-pink-900 gap-4">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-3 w-full sm:w-auto">
                          <h3 className="text-sm font-semibold text-slate-900 dark:text-white flex-1 sm:flex-none">📊 Questions to be Generated</h3>
                          {/* BTL Customization toggle */}
                          <button
                            type="button"
                            onClick={() => setBtlCustomization(prev => !prev)}
                            className={`inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-all shadow-sm w-full sm:w-auto ${
                              btlCustomization
                                ? 'bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow-lg shadow-purple-500/40 sm:scale-105'
                                    ...
                                    return (
                                      <td key={pIdx} className="px-3 py-2 text-center align-top">
                                        <div className="space-y-2">
                                          {btlCustomization ? (
                                            <>
                                              {/* MCQ / Descriptive grid summary */}
                                              <div className="grid grid-cols-3 gap-1 border border-pink-100 dark:border-pink-900/40 rounded-md p-1 bg-white dark:bg-slate-800/40 text-xs mb-1 min-w-[130px]">
                                                <div className="flex flex-col items-center">
                                                  <span className="text-[8px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Total</span>
                                                  {isEditingPattern ? (
                                                    <input
                                                      type="number" min={0}
                                                      value={pc.questionCount || ''}
                                                      onChange={e => updateLocalUnitPart(uStr, pIdx, 'questionCount', e.target.value === '' ? 0 : Number(e.target.value))}
                                                      className="w-10 text-center text-xs font-bold text-slate-800 dark:text-white border border-pink-200 dark:border-pink-800 rounded bg-white dark:bg-slate-800 focus:outline-none focus:ring-1 focus:ring-pink-400 py-0.5"
                                                    />
                                                  ) : pc.questionCount === 0 ? (
                                                    <span className="text-xs italic text-slate-400 dark:text-slate-500">auto</span>
                                                  ) : (
                                                    <span className="text-xs font-bold text-slate-800 dark:text-white">{pc.questionCount}</span>
                                                  )}
                                                </div>
                                                <div className="flex flex-col items-center">
                                                  <span className="text-[8px] font-bold text-pink-500 dark:text-pink-400 uppercase tracking-wide">MCQ</span>
                                                  {isEditingPattern ? (
                                                    <input
                                                      ...
                                                      className="w-10 text-center text-xs font-semibold border border-pink-200 dark:border-pink-800 rounded bg-pink-50 dark:bg-pink-900/30 text-pink-700 dark:text-pink-300 focus:outline-none focus:ring-1 focus:ring-pink-400 py-0.5"
                                                    />
                                                  ) : (
                                                ...
                                                <div className="flex flex-col items-center">
                                                  <span className="text-[8px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Desc</span>
                                            ...
                                            <>
                                              {/* Standard inner stats grid summary */}
                                              <div className="grid grid-cols-3 gap-1 border border-pink-100 dark:border-pink-900/40 rounded-md p-1 bg-white dark:bg-slate-800/40 text-xs min-w-[130px]">
                                                <div className="flex flex-col items-center">
                                                  <span className="text-[8px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Total</span>
                                                  {isEditingPattern ? (
                                                    <input
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 w-full">
                  <div className="min-w-0 flex-1">
                    <p className="font-semibold text-slate-900 dark:text-white group-hover:text-amber-700 dark:group-hover:text-amber-300 transition-colors break-words">
                  ...
                  <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto sm:justify-end" onClick={e => e.stopPropagation()}>
          ...
          <div className="space-y-3">
            {questionBanks.map(bank => (
              <div
                key={bank.id}
                onClick={() => setViewingBank(bank)}
                className={`flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-white dark:bg-slate-900 border-2 rounded-xl cursor-pointer transition-all group gap-4 ${
                  bank.id === latestBankId
                    ...
                    : 'border-pink-200 dark:border-pink-700 hover:border-pink-300 dark:hover:border-pink-600 animate-in fade-in'
                }`}
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-450 mt-1 break-words">
                    {getSubjectName(bank.subject_id)} • {new Date(bank.created_at).toLocaleDateString()}
                  </p>
                </div>
                <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto sm:justify-end" onClick={e => e.stopPropagation()}>
      ...
      {showSuccessPopup && successBank && (
        ...
        >
          <div
            className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-8 max-w-[95vw] sm:max-w-sm w-full mx-4 text-center border-2 border-pink-300 dark:border-pink-700"
            onClick={e => e.stopPropagation()}
          >
      ...
      <ConfirmationModal
        ...
        onClose={() => setConfirmDelete({ open: false, bankId: null })}
        isDangerous={true}
      />
      ...
      {shareBankId && (
        <MobileShareModal
          ...
          onShare={async (bankId, emails) => {
            setShareLoading(true);
            setShareResult(null);
            try {
              const res = await questionBankApi.share(bankId, { recipient_emails: emails });
              setShareResult(res.data as any);
            } catch (err: any) {
...
function MobileShareModal({
  bank, subjects, onClose, onShare, sharing, shareResult
}: {
  bank: any;
  subjects: any[];
  onClose: () => void;
  ...
  sharing: boolean;
  shareResult: { shared_with: string[]; failed: string[]; drive_link?: string | null } | null;
}) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-900 rounded-t-3xl sm:rounded-2xl w-full sm:max-w-lg shadow-2xl border border-purple-200 dark:border-purple-700 overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-purple-100 dark:border-purple-800 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20">
          ...
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors flex items-center justify-center">
            <X className="w-5 h-5" />
          </button>
        </div>

        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto pb-safe">
          {shareResult ? (
            <div className="space-y-3">
              {shareResult.shared_with.length > 0 && (
                <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl">
                  <p className="font-semibold text-emerald-700 dark:text-emerald-300 flex items-center gap-2">
                    <Check className="w-4 h-4" /> Shared via Share Sheet!
                  </p>
                  <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-1">The Excel file was sent to the selected app.</p>
                </div>
              )}
              <button onClick={onClose} className="btn btn-primary w-full flex items-center justify-center">Done</button>
            </div>
          ) : (
            <>
              <div className="py-4 text-center">
                <p className="text-slate-600 dark:text-slate-300 text-sm">
                  Tap to share the Question Bank file via your device's native sharing options.
                </p>
              </div>
              <div className="flex gap-3 pt-2">
                <button onClick={onClose} className="btn btn-secondary flex-1 flex items-center justify-center">Cancel</button>
                <button
                  onClick={() => onShare(bank.id, [])}
                  disabled={sharing}
                  className="btn btn-primary flex-1 bg-gradient-to-r from-purple-600 to-pink-600 border-0 disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {sharing ? <><Loader2 className="w-4 h-4 animate-spin" /> Sharing...</> : <><Share2 className="w-4 h-4 text-white" /> Share</>}
                </button>
              </div>
            </>
          )}
