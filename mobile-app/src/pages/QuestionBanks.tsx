...
export default function QuestionBanks() {
  ...
  const { preferredProvider, customKeys, customModels, useLocalModels, providerKeys } = useAISettingsStore();
  ...
  const offlineBanks = useOfflineStore(state => state.offlineQuestionBanks);
  const displayBanks = [...offlineBanks, ...questionBanks];
  ...
  const loadData = async () => {
    try {
      ...
      localStorage.setItem('offline_syllabi', JSON.stringify(syllabusRes.data));
      localStorage.setItem('offline_question_banks', JSON.stringify(banksRes.data));
      localStorage.setItem('offline_subjects', JSON.stringify(subjectsRes.data || []));

      try {
        const assignmentsRes = await staffApi.getMySubjects();
        setMyAssignments(assignmentsRes.data || []);
        localStorage.setItem('offline_assignments', JSON.stringify(assignmentsRes.data || []));
      } catch (err) {
        // Ignored
      }
    } catch (err: any) {
      ...
      const cachedSyllabiStr = localStorage.getItem('offline_syllabi');
      const cachedBanksStr = localStorage.getItem('offline_question_banks');
      const cachedSubjectsStr = localStorage.getItem('offline_subjects');
      const cachedAssignmentsStr = localStorage.getItem('offline_assignments');
      
      if (cachedSyllabiStr && cachedBanksStr && cachedSubjectsStr) {
        const cachedSyllabi = JSON.parse(cachedSyllabiStr);
        const cachedBanks = JSON.parse(cachedBanksStr);
        const cachedSubjects = JSON.parse(cachedSubjectsStr);
        
        const syllabusMap: Record<string, Syllabus> = {};
        cachedSyllabi.forEach((s: Syllabus) => { syllabusMap[s.subject_id] = s; });
        setSyllabi(syllabusMap);
        setQuestionBanks(cachedBanks);
        setSubjects(cachedSubjects);
        
        if (cachedAssignmentsStr) {
          setMyAssignments(JSON.parse(cachedAssignmentsStr));
        }
        
        setError('Offline Mode: Displaying cached data. You can generate offline question banks using Gemma.');
      } else {
        if (!err.response) {
          setError('Server is unreachable and no cached data is available.');
  ...
  const buildGeneratePayload = (syllabus: Syllabus) => {
    ...
    // Filter custom keys to only send those where the user selected 'custom' source
    const activeCustomKeys = Object.fromEntries(
      Object.entries(customKeys || {}).filter(([provider]) => providerKeys[provider] === 'custom')
    );
  ...
  const handleGenerateOffline = async (syllabus: Syllabus) => {
    const { preferredProvider, useLocalModels } = useAISettingsStore.getState();
    
    if (preferredProvider !== 'local' && !useLocalModels) {
      setError('You are offline. Please go to AI Settings, set provider to Gemma (Local Offline), and make sure it is downloaded.');
      setGenerating(false);
    ...
    try {
      ...
      const subjectCode = subject?.code || 'SUBJ';
      
      const payload = buildGeneratePayload(syllabus);
      const selectedUnits = (syllabus.units || []).filter(u => selectedUnitIds.includes(u.unitNumber));
      
      const offlinePayload = {
        syllabus_units: selectedUnits,
        parts: localParts.map(p => sanitizePartConfig(p, btlCustomization)),
        subject_name: subjectName,
        cdap_units: null,
        include_answers: includeAnswers,
        preferred_provider: payload.preferred_provider,
        custom_keys: payload.custom_keys,
        custom_models: payload.custom_models,
        use_local_models: payload.use_local_models
      };

      setGlobalLoading(true, 'Generating Question Bank Offline using Gemma (This may take a minute...)');
      const response = await questionBankApi.generateOffline(offlinePayload);
      
      if (response.data && response.data.success) {
        const randId = `offline-${Date.now()}-${Math.round(Math.random() * 100000)}`;
        const offlineBank: any = {
          id: randId,
          subject_id: selectedSubjectId,
          syllabus_id: syllabus.id,
          title: `${subjectCode} Question Bank (Offline)`,
          questions: {
            parts: response.data.questions,
            include_answers: includeAnswers
          },
          status: 'APPROVED',
          is_offline: true,
          ...
          sync_payload: {
            subject_id: selectedSubjectId,
            syllabus_id: syllabus.id,
            pattern_id: selectedSubjectPattern?.id || undefined,
            title: `${subjectCode} Question Bank`,
            questions: response.data.questions,
            include_answers: includeAnswers
          }
        };

        useOfflineStore.getState().addOfflineBank(offlineBank);
        onBankCreated(offlineBank);
      } else {
        throw new Error('Offline generation failed.');
      }
    } catch (err: any) {
      console.error(err);
      setError(err.response?.data?.detail || err.message || 'Offline generation failed.');
    } finally {
      setGenerating(false);
      setGlobalLoading(false);
    }
  ...
  const handleGenerate = async () => {
    ...
    if (!netStatus.connected) {
      await handleGenerateOffline(syllabus);
      return;
    }
  ...
  const handleDownload = async (bank: QuestionBank) => {
    ...
    try {
      let response;
      if ((bank as any).is_offline) {
        const subject = subjects.find(s => s.id === bank.subject_id);
        const subjectCode = subject?.code || 'SUBJ';
        const subjectName = subject?.name || 'Subject';
        response = await questionBankApi.exportBytes({
          questions: bank.questions?.parts || {},
          subject_name: subjectName,
          subject_code: subjectCode,
          parts_config: selectedSubjectPattern?.parts || [],
          include_answers: (bank.questions as any)?.include_answers !== false
        });
      } else {
        response = await questionBankApi.download(bank.id);
      }
      ...
      onConfirm: async () => {
        try {
          if (id.startsWith('offline-')) {
            useOfflineStore.getState().removeOfflineBank(id);
          } else {
              ...
              >
                <div className="min-w-0 space-y-1">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {(bank as any).is_offline && (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-gradient-to-r from-amber-500 to-orange-500 text-white shadow-sm">
