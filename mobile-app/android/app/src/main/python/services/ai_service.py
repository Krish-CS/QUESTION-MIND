...
class AIService:
    ...
    def _build_custom_provider_chain(self, ai_settings: Optional[Dict[str, Any]]) -> List[Dict]:
        if not ai_settings:
            return self.provider_chain

        from ..config import settings
        chain = []
        
        pref_provider = ai_settings.get("preferred_provider", "backend")
        provider_keys = ai_settings.get("provider_keys", {})
        custom_keys = ai_settings.get("custom_keys", {})

        def get_key_for_provider(prov_name: str) -> Optional[str]:
            source = provider_keys.get(prov_name, "system-1")
            if pref_provider == "custom":
                source = "custom"
            ...
            elif source == "system-2":
                if prov_name == "cerebras":
                    return settings.CEREBRAS_API_KEY_2
                elif prov_name == "groq":
                    return settings.GROQ_API_KEY
            elif source == "custom":
                return custom_keys.get(prov_name)
            return None

        # Priority 1: Cerebras
        c_key = get_key_for_provider("cerebras")
        if c_key:
            chain.append({
                "name": "cerebras",
                "api_key": c_key,
        ...
        c_key_2 = settings.CEREBRAS_API_KEY_2 if provider_keys.get("cerebras", "system-1") == "system-1" and pref_provider != "custom" else None
        if c_key_2:
            chain.append({
                "name": "cerebras-2",
                "api_key": c_key_2,
        ...
        # Priority 2: Groq
        g_key = get_key_for_provider("groq")
        if g_key:
            chain.append({
                "name": "groq",
                "api_key": g_key,
        ...
        # Priority 3: NVIDIA
        nv_key = get_key_for_provider("nvidia")
        if nv_key:
            chain.append({
                "name": "nvidia",
                "api_key": nv_key,
        ...
        # Priority 4: OpenRouter
        or_key = get_key_for_provider("openrouter")
        if or_key:
            chain.append({
                "name": "openrouter",
                "api_key": or_key,
    ...
    async def generate_questions(
        ...
        provider_chain = self._build_custom_provider_chain(ai_settings)
        if not provider_chain:
...
[{{"question":"...","unit":1{cdap_field},"btl":"BTL2","marks":{marks},"answer":"..."}}]"""
    ...
    async def generate_full_question_bank(
        ...
        for i, part in enumerate(parts, 1):
            ...
            questions = await self.generate_questions(syllabus_units, part, subject_name, cdap_units, ai_settings)
