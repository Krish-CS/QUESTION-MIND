...
class SyllabusParser:
    ...
    def _parse_pdf(self, file_path: str) -> List[Dict]:
        ...
        if pdfplumber is None:
            raise Exception("PDF parsing is not supported on this device. Please use the web version.")
        try:
            full_text = ""
    ...
    def _parse_docx(self, file_path: str) -> List[Dict]:
        """Parse DOCX syllabus"""
        if Document is None:
            raise Exception("DOCX parsing is not supported on this device. Please use the web version.")
        try:
