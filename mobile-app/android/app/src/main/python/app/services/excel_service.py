...
class ExcelService:
    def __init__(self):
        self.upload_dir = settings.upload_question_banks_dir
    ...
    def generate_question_bank_excel(
        ...
        for part_name, part_questions in self._unit_major_runs(questions, parts_config):
            ...
            for q in part_questions:
                ...
                if image_data:
                    try:
                        if image_data.startswith('/api/question-bank/images/') or image_data.startswith('/api/static/images/'):
                            filename = image_data.split('/')[-1]
                            if '/' not in filename and '\\' not in filename and '..' not in filename:
                                img_path = os.path.normpath(os.path.join(settings.upload_images_dir, filename))
